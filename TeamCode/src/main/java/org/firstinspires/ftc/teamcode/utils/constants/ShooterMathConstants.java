package org.firstinspires.ftc.teamcode.utils.constants;

import com.acmerobotics.dashboard.config.Config;

@Config
public class ShooterMathConstants {
    public static double SCORE_HEIGHT = 34;
    public static double SCORE_ANGLE = -0.1;
    public static double PASS_THROUGH_POINT_RADIUS = 2;
    public static double HOOD_MAX_ANGLE = Math.toRadians(55.861336); //0.32 servo pos
    public static double HOOD_MIN_ANGLE = Math.toRadians(37.288274); //0.17 servo pos
    public static double perpMultiplier = 3.5;
    public static double turretMultiplier = 0.5;
}
